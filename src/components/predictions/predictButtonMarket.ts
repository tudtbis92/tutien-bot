...
export async function handlePredictButtonMarket(interaction: ButtonInteraction): Promise<void> {
  const customId = interaction.customId; // predict:{betType}:{matchId}:{prediction}
  const parts = customId.split(':');
  const betType = parts[1] as 'score' | 'over_under' | 'spread';
  const matchId = parseInt(parts[2] || '', 10);
  const prediction = parts[3] || '';

  if (isNaN(matchId) || !prediction) {
    const locale = resolveLocale(undefined, interaction.locale);
    const t = getT(locale);
    await interaction.reply({
      embeds: [buildErrorEmbed(t('football:errors.invalid_market', 'Lựa chọn đặt cược không hợp lệ.'))],
      ephemeral: true,
    });
    return;
  }
  ...
  const modal = new ModalBuilder()
    .setCustomId(`predict:modal:${betType}:${matchId}:${prediction}`)
