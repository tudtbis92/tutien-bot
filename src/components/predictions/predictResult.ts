...
export async function handlePredictResult(interaction: StringSelectMenuInteraction): Promise<void> {
  const customId = interaction.customId; // predict:result:{matchId}
  ...
  if (isNaN(matchId)) {
    ...
    await interaction.reply({
      embeds: [buildErrorEmbed(t('football:errors.invalid_match', 'Invalid match selected.'))],
      ephemeral: true,
    });
    return;
  }
  ...
  const modal = new ModalBuilder()
    .setCustomId(`predict:modal:result:${matchId}:${prediction}`)
