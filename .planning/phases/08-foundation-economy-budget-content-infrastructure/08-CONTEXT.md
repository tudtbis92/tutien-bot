...
- **D-21:** **Sanguo emoji are ALL animated → render via `<a:name:id>` markup.** Verified against the live app (2026-08-12, Discord API): 1056/1056 sanguo emojis are animated (GIF). `heroEmoji()` emits `<a:${key}:${id}>`; `<:...>` (missing `a:`) renders as literal text in Discord. Generator template (`scripts/gen-sanguo-emojis.ts`) and committed file both emit the `a:` prefix. — **Reversibility:** one-way — changing to static markup would break ALL emoji rendering (SC3 regression; user confirmed literal `:dtr_t0:` on first deploy).
- **D-22:** **Zone markers render in message CONTENT with `# ` (H1) headers, not embed fields.** Discord markdown headings work ONLY in message content — they do NOT render inside `embed.fields[*].value` or `embed.description` (confirmed via `discord/discord-api-docs#7167` + Discord blog, 2026-08-12). `# ` is the largest header (H1); `##`/`###`/`-#` are smaller. Emoji inside H1 headers render larger → zone markers moved to `content` with `# ` prefix; embed keeps current-position + node list (D-15 follow-up). — **Reversibility:** reversible.
